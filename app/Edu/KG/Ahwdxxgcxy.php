...
class Ahwdxxgcxy extends EduProvider implements EduParserInterface
{
    /**
     * 相关网络地址
     *
    ...
    protected static $url = [
        'base'        => 'http://218.22.58.76:2346/', //根域名
    ...
    public function parserScoresInfo($html)
    {
        try {
            ...
            foreach ($table as $tableIndex => $tableNode) {
                ...
                if (preg_match('/^\[(.*)\](.*)/', $tdText2, $courseMatches)) {
                    $tempCourse = [
    ...
    public function parserTablesHiddenValue($html)
    {
        ...
        if (is_array($xnxq)) {
            foreach ($xnxq as $xn) {
                ...
                $hidsjyzm = strtoupper(md5('12810' . $xn . $randostr));
            ...
            $randostr = $this->randomStr();
            $hidsjyzm = strtoupper(md5('12810' . $xnxq . $randostr));
    ...
    {
        return strtoupper(substr(md5($assist . strtoupper(substr(md5($plaintext), 0, 30)) . '12810'), 0, 30));
    }
